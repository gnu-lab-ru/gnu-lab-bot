...
(ert-deftest changelog-fallback-categorization ()
  (cl-letf (((symbol-function 'gnu-lab-changelog--call-git)
             (lambda (&rest args)
               (cond
                ;; subjects for fallback
                ((member "--pretty=format:%s" args)
                 (mapconcat #'identity
                            '("feat: add X"
                              "fix: crash on Y"
                              "docs: update README"
